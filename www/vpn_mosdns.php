...
$tab_array = [
    ...
    2 => [gettext("MosDNS"), true, "vpn_mosdns.php"],	
    3 => [gettext("Sub"), false, "vpn_sub.php"],
];

display_top_tabs($tab_array);

function mosdns_exec($command, &$output = null, &$return_var = null)
{
    $output = [];
    $return_var = 0;
...
function mosdns_exec_background($command)
{
    $nohup = '/usr/bin/nohup';
    $shell = '/bin/sh';
...
function handleServiceAction($action)
{
    ...
    mosdns_exec_background('/usr/sbin/service mosdns ' . escapeshellarg($action));
...
function mosdns_csrf_token_field()
{
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div id="mosdns-status" class="alert alert-info" style="margin-bottom: 0;">
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form method="post" class="form-inline" id="mosdns-service-form" style="margin: 6px 0 10px 10px;">
            <?php mosdns_csrf_token_field(); ?>
            ...
            <input type="hidden" name="ajax" value="0" id="mosdns-ajax-flag">
        </form>
    </div>
</div>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form method="post" id="mosdns-config-form">
            <?php mosdns_csrf_token_field(); ?>
            ...
            <input type="hidden" name="ajax" value="0" id="mosdns-config-ajax-flag">
        </form>
    </div>
</div>
...
<script>
(function() {
    var statusElement = document.getElementById('mosdns-status');
    var logViewer = document.getElementById('log-viewer');
    var logErrorShown = false;
    var serviceForm = document.getElementById('mosdns-service-form');
    var ajaxFlag = document.getElementById('mosdns-ajax-flag');
    var configForm = document.getElementById('mosdns-config-form');
    var configAjaxFlag = document.getElementById('mosdns-config-ajax-flag');
    ...
    function scheduleStatusRefresh() {
        window.setTimeout(checkMosdnsStatus, 1500);
        window.setTimeout(checkMosdnsStatus, 3000);
        window.setTimeout(checkMosdnsStatus, 5000);
        window.setTimeout(checkMosdnsStatus, 8000);
    }
        ...
        fetch('status_mosdns.php', {
            ...
            .then(function(data) {
                if (data.status === 'running') {
                    setStatus('<i class="fa fa-check-circle"></i> mosdns 正在运行', 'alert-success');
                } else {
                    setStatus('<i class="fa fa-times-circle"></i> mosdns 已停止', 'alert-danger');
                }
            })
    ...
    function submitServiceAction(button) {
        ...
        if (button.value === 'start') {
            setStatus('<i class="fa fa-circle-o-notch fa-spin"></i> 正在启动 mosdns...', 'alert-info');
        } else if (button.value === 'stop') {
            setStatus('<i class="fa fa-circle-o-notch fa-spin"></i> 正在停止 mosdns...', 'alert-warning');
        } else if (button.value === 'restart') {
            setStatus('<i class="fa fa-circle-o-notch fa-spin"></i> 正在重启 mosdns...', 'alert-info');
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        checkMosdnsStatus();
        refreshLogs();
        window.setInterval(checkMosdnsStatus, 5000);
